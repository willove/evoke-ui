<template>
  <div class="kg-page">
    <eb-page-header title="知识图谱" subtitle="知识节点的关联结构与跳转关系">
      <template #actions>
        <eb-segmented v-model="scope" :options="[{ label: '全部关联', value: 'all' }, { label: '仅一级', value: 'direct' }]" size="small" />
      </template>
    </eb-page-header>

    <eb-row :gutter="16" class="kg-block">
      <!-- 左：关系图（固定布局 SVG，节点可点选） -->
      <eb-col :xs="24" :lg="14">
        <eb-section-card title="关联结构" :padding="false" class="kg-graph-card">
          <div class="kg-graph">
      ...
      <!-- 右：节点详情与关联列表 -->
      <eb-col :xs="24" :lg="10">
        <eb-section-card :padding="false" class="kg-detail-card">
          <template #header>
            <div class="kg-detail-title">
              <span>{{ selected.label }}</span>
              <eb-tag size="small" effect="plain">{{ groupLabel(selected.group) }}</eb-tag>
            </div>
          </template>
          <div class="kg-detail-body">
            <eb-detail-descriptions :column="1" :border="false" :data="selectedInfo" :items="infoItems" />
            ...
            <div v-if="relatedDocs.length" class="kg-docs">
              <a v-for="doc in relatedDocs" :key="doc.id" class="kg-doc" @click="EbMessage.info(`示例：阅读「${doc.title}」`)">
                {{ doc.title }}
              </a>
            </div>
...
.kg-graph {
  ...
  height: 400px;
  background:
    linear-gradient(var(--eb-border-color-extra-light, #f0f1f3) 1px, transparent 1px),
    linear-gradient(90deg, var(--eb-border-color-extra-light, #f0f1f3) 1px, transparent 1px);
...
.kg-node {
  ...
  border: 1px solid var(--eb-border-color, #c8ccd2);
  border-radius: 999px;
  cursor: pointer;
  transition: all 0.2s;
}
...
.kg-node--core {
  ...
  color: #fff;
  background: var(--eb-color-primary, #175dff);
  border-color: var(--eb-color-primary, #175dff);
}
.kg-node.is-active {
  border-color: var(--eb-color-primary, #175dff);
...
.kg-legend__dot--module {
  background: var(--eb-color-success, #22a45d);
}
.kg-legend__dot--topic {
  background: var(--eb-border-color, #c8ccd2);
}
.kg-detail-card {
  min-height: 480px;
}
