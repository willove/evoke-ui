import { expect, test } from '@playwright/test'
import { settle } from './helpers.mjs'

/**
 * charts3d 站（docs-charts-3d/）视觉基线 — 精选稳定页面 + 暗色轮次
 * 三维图为 canvas 入场动画（rAF 驱动），settle 需加长余量等全部画完；
 * 暗色走 localStorage 'cd-dark'，DocLayout 挂载时统一应用 html.dark
 * （初始化脚本先于页面脚本运行，图表首帧即按暗色渲染）。
 */
const PAGES = [
  ['home', '/'],
  ['chart-bar3d', '/chart/bar3d'],
  ['chart-surface3d', '/chart/surface3d'],
  ['chart-pie3d', '/chart/pie3d'],
  ['chart-api', '/chart/api'],
]

const DARK_PAGES = [
  ['chart-bar3d', '/chart/bar3d'],
  ['chart-surface3d', '/chart/surface3d'],
]

async function shoot(page, path, name) {
  await page.goto(path, { waitUntil: 'networkidle' })
  await settle(page, { extraMs: 1800 }) // 覆盖进场动画（默认 600ms）+ 布局收敛
  await expect(page).toHaveScreenshot(`${name}.png`, { fullPage: true })
}

for (const [name, path] of PAGES) {
  test(`charts3d 站 · ${name}`, async ({ page }) => {
    await shoot(page, path, name)
  })
}

for (const [name, path] of DARK_PAGES) {
  test(`charts3d 站 · 暗色 ${name}`, async ({ page }) => {
    await page.addInitScript(() => localStorage.setItem('cd-dark', '1'))
    await shoot(page, path, `dark-${name}`)
  })
}
