 ...
 * 会话区滚动容器契约（样式级钉子）
 *
 * jsdom 没有布局，滚不滚得动测不出来——「新消息不自动滚到底」那次的根因是
 * 外层 .eb-ai-console__chat 在滚、内层 .eb-chat-list 没被撑满，而 ChatList 的
 * 跟随逻辑驱动的是内层。唯一可测的落点就是这条样式契约：
 * __chat 必须是 flex 列容器，内层列表才有 flex:1 可吃、才成为唯一滚动元素。
 */
describe('会话区滚动容器契约', () => {
  const css = readFileSync(
    resolve(dirname(fileURLToPath(import.meta.url)), '../src/components/ai-console/style.css'),
    'utf8',
  )
  const block = css.slice(
    css.indexOf('.eb-ai-console__chat {'),
    css.indexOf('}', css.indexOf('.eb-ai-console__chat {')),
  )

  it('__chat 是 flex 列容器（内层列表才能吃满高度）', () => {
    expect(block).toContain('display: flex')
    expect(block).toContain('flex-direction: column')
  })

  it('__chat 不再自带左右内边距（内层列表已有，避免双重内缩）', () => {
    expect(block).not.toMatch(/padding:\s*0\s+var\(--eb-space-4\)/)
  })
})
