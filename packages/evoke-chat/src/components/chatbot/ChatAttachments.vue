<template>
  <div v-if="attachments && attachments.length" class="eb-chat-attachments">
    ...
    >
      ...
      <div
        v-else
        class="eb-chat-attachments__icon"
        :class="colored ? toneClass(file) : ''"
        aria-hidden="true"
      >
        <!-- 类型与配色都按后缀匹配（pdf/word/excel/ppt/zip/image/music/video/code…），见 fileIcons.js -->
        <eb-icon :name="fileIconFor(file)" />
      </div>
...
import { fileIconFor, fileIconToneFor } from "./fileIcons";
...
const props = defineProps({
  ...
  /** 图标是否按类型染色（默认染；关掉则统一跟随正文色，适合极简/单色版式） */
...
/** 色调 → 类名（空色调返回空串，图标跟随正文色） */
function toneClass(file) {
  const tone = fileIconToneFor(file);
