<template>
  ...
  >
    ...
    <div class="eb-chat-sender__input-wrapper">
      ...
      <!-- 工具行与发送行并作一行：三行结构在小高度容器里会把正文挤没（见 2026-09-22 高度协调） -->
...
// 占位文案在渲染期取：语言包要跟着 locale 走，而 prop 默认值在模块加载时就冻结了
const placeholderText = computed(() => props.placeholder ?? labels.sender.placeholder);
