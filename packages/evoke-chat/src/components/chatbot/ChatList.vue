<template>
  ...
  >
    ...
    <!-- 回底按钮：外层是 0 高的 sticky 锚点，滚动时钉在可视区底部
         （此前是 position:absolute，会跟着内容一起滚走，看着像"没悬浮"） -->
    <transition name="eb-chat-list__backtop-fade">
      <div v-show="showBackToBottom" class="eb-chat-list__backtop-wrap">
        <button 
...
import EbVirtualList from "@wil-works/evoke-business-ui/virtual-list";
...
const props = defineProps({
  ...
  /** 消息头部：头像 / 显示名可整块隐去；传对象可分侧（{ user: false } / { assistant: false }） */
