<template>
  <div :class="['ew-load-more', `is-${status}`]">
    <!-- 触底哨兵：autoLoad 时用 IntersectionObserver 自动触发 -->
    <div v-if="autoLoad && !disabled" ref="sentinelRef" class="ew-load-more__sentinel" />
    <button
      type="button"
    ...
    >
      <slot name="loading" v-if="status === 'loading'">
        <span class="ew-load-more__spinner" />
      </slot>
      <slot :status="status">
        <span class="ew-load-more__text">{{ statusText }}</span>
      </slot>
    </button>
  </div>
</template>
