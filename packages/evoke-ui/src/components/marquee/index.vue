<template>
  <div
    ...
    role="marquee"
  >
    <div class="ew-marquee__track">
      <template v-if="textMode">
        <div v-for="g in 2" :key="g" class="ew-marquee__group" :aria-hidden="g === 2 ? 'true' : undefined">
          <template v-for="(w, i) in items" :key="i">
            <span class="ew-marquee__text" :class="{ 'is-outline': alternateOutline && i % 2 === 1 }">{{ w }}</span>
            <EwIcon v-if="separator" :name="separator" :size="separatorSize" class="ew-marquee__separator" />
          </template>
        </div>
      </template>
      <template v-else>
        <div class="ew-marquee__group"><slot /></div>
        <div class="ew-marquee__group" aria-hidden="true"><slot /></div>
      </template>
    </div>
    <div class="ew-marquee__mask" aria-hidden="true" />
  </div>
</template>
