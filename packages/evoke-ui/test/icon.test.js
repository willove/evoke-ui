...
import EwIcon from '../src/components/icon/index.vue'
import { registerIcons, getIconByName, getIconNames, hasIcon, defineComponent, h } from './helpers'

describe('EwIcon / 图标注册表', () => {
  it('核心集渲染内置 SVG', () => {
    const wrapper = mount(EwIcon, { props: { name: 'search', size: 20 } })
    expect(wrapper.classes()).toContain('ew-icon')
  ...
  it('size 支持字符串', () => {
    const wrapper = mount(EwIcon, { props: { name: 'search', size: '1.5em' } })
    expect(wrapper.attributes('style')).toContain('font-size: 1.5em')
  })

  it('color 覆写', () => {
    const wrapper = mount(EwIcon, { props: { name: 'search', color: 'red' } })
    expect(wrapper.attributes('style')).toContain('color: red')
  })

  it('未知名称回退到插槽', () => {
    const wrapper = mount(EwIcon, {
  ...
  it('registerIcons 注册自定义组件并可解析', () => {
    ...
    const wrapper = mount(EwIcon, { props: { name: 'my-custom-icon' } })
  ...
  it('loadShowcaseIcons 加载展示集后可按 Remix 原生名渲染', async () => {
    ...
    const wrapper = mount(EwIcon, { props: { name: 'brush-line' } })
