/**
 * 类型产物后处理（vue-tsc 之后运行）—— 与底座 post-types.mjs 同构，
 * 差异仅在入口清单来自本包的 getEtComponentEntries()（tools-ui 计划 02 §二）。
 *
...
console.log(
  `[post-types] ${entries.length} 个子路径存根 → dist/types/entries/；说明符改写 ${rewritten} 个文件`,
)
