...
import { chart3dOptionsSchema, validateOptions3d } from '../src/schema.js'
import { CHART3D_PALETTES, CHART3D_RAMPS, resolveChartPalette, resolveChartRamp } from '../src/palette.js'
import { applySeriesPalette, clearSeriesPalette, SLOT_COUNT } from '../src/paletteApply.js'
import { getTheme, CHART3D_TYPES, easings, resolveEasing, resolveI18n } from '../src/types.js'
...
describe('色系注入', () => {
  it('数组写入 8 个内联令牌并可清除', () => {
    ...
    expect(document.documentElement.style.getPropertyValue(`--ev-color-series-${SLOT_COUNT}`)).toBe('#333333')
    clearSeriesPalette()
    expect(document.documentElement.style.getPropertyValue('--ev-color-series-1')).toBe('')
  })

  it('按 id 注入内置色系', () => {
    expect(applySeriesPalette('aurora', { isDark: true })).toBe(true)
  ...
  it('明暗两套注入 style 表并随 dark 切换', () => {
    ...
    const style = document.head.querySelector('style[data-charts-3d-palette]')
    expect(style).toBeTruthy()
    expect(style.textContent).toContain('--ev-color-series-1:#aaaaaa;')
    expect(style.textContent).toContain('html.dark{--ev-color-series-1:#bbbbbb;')
    clearSeriesPalette()
    expect(document.head.querySelector('style[data-charts-3d-palette]')).toBe(null)
  })
