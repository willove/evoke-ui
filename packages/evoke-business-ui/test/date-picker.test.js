...
import EvDatePicker from '../src/components/date-picker/index.vue'
...
function mountPicker(props = {}) {
  ...
  return { wrapper, picker: () => wrapper.findComponent(EvDatePicker) }
}
...
function dayCell(dayNum, tableIndex = 0) {
  const tables = document.querySelectorAll('.ev-date-table')
...
function monthCell(monthIdx) {
  const tds = [...document.querySelectorAll('.ev-month-table td')]
  return tds[monthIdx]
}

function yearCellInDecade(year) {
  const tds = [...document.querySelectorAll('.ev-year-table td')]
...
describe('EvDatePicker 渲染契约', () => {
  it('单值编辑器双 class + ev-input 同构 DOM', () => {
    const { wrapper } = mountPicker()
    expect(wrapper.classes()).toContain('ev-date-editor')
    expect(wrapper.classes()).toContain('ev-date-editor')
    expect(wrapper.find('.ev-input__wrapper').exists()).toBe(true)
    expect(wrapper.find('.ev-input__inner').exists()).toBe(true)
    expect(wrapper.find('.ev-input__prefix').exists()).toBe(true)
    expect(wrapper.find('.ev-input__inner').attributes('placeholder')).toBe('选择日期')
  })
  ...
  it('区间编辑器：双输入 + 分隔符 + 独立 placeholder', () => {
    ...
    expect(wrapper.find('.ev-range-separator').text()).toBe('-')
    expect(inputs[0].attributes('placeholder')).toBe('开始日期')
    expect(inputs[1].attributes('placeholder')).toBe('结束日期')
    expect(wrapper.classes()).toContain('ev-date-editor--daterange')
  })

  it('datetime placeholder 组合', () => {
    const { wrapper } = mountPicker({ type: 'datetime' })
    expect(wrapper.find('.ev-input__inner').attributes('placeholder')).toContain('选择日期')
  })
})

describe('EvDatePicker 面板与选择（date）', () => {
  it('打开面板：ev-picker__popper + ev-date-picker + 6×7 网格', async () => {
    const { wrapper } = mountPicker()
    await openPanel(wrapper)
    expect(document.querySelector('.ev-picker__popper')).toBeTruthy()
    expect(document.querySelector('.ev-picker-panel.ev-date-picker')).toBeTruthy()
    expect(document.querySelectorAll('.ev-date-table td').length).toBe(42)
    expect(document.querySelectorAll('.ev-date-table th').length).toBe(7)
  })
  ...
  it('点击日期：update/change 传 Date（当天零点）并关闭', async () => {
    ...
    expect(document.querySelector('.ev-picker__popper')).toBeNull()
  })
  ...
  it('初始 Date 值回显 + 表格 current 高亮', async () => {
    ...
    expect(wrapper.find('.ev-input__inner').element.value).toBe(
      dayjs(d).format('YYYY-MM-DD')
    )
    await openPanel(wrapper)
    const current = document.querySelector('.ev-date-table td.current')
  ...
  it('shortcuts 快捷选项应用并关闭', async () => {
    ...
    await openPanel(wrapper)
    expect(document.querySelector('.ev-picker-panel__sidebar')).toBeTruthy()
    document.querySelector('.ev-picker-panel__shortcut').click()
    await flush()
    const emitted = picker().emitted('update:modelValue')[0][0]
  ...
  it('视图切换：月份视图选月回到日期视图', async () => {
    ...
    await openPanel(wrapper)
    // 月份标签（第二个）→ 月份视图
    document.querySelectorAll('.ev-date-picker__header-label')[1].click()
    await flush()
    expect(document.querySelectorAll('.ev-month-table td').length).toBe(12)
    monthCell(0).click() // 1 月
    await flush()
    expect(document.querySelector('.ev-date-table')).toBeTruthy()
    const label = document.querySelector('.ev-date-picker__header-label').textContent
  ...
  it('年份视图：十年网格 + 点击年份进入月份视图', async () => {
    const { wrapper } = mountPicker()
    await openPanel(wrapper)
    const labels = document.querySelectorAll('.ev-date-picker__header-label')
    labels[0].click() // 年份标签 → 年视图
    await flush()
    const tds = document.querySelectorAll('.ev-year-table td')
    expect(tds.length).toBe(12)
    expect(document.querySelector('.ev-year-table td.prev-decade')).toBeTruthy()
    expect(document.querySelector('.ev-year-table td.next-decade')).toBeTruthy()
    yearCellInDecade(2026).click()
    await flush()
    expect(document.querySelector('.ev-month-table')).toBeTruthy()
  })
})

describe('EvDatePicker month/year 类型', () => {
  ...
  it('year：十年网格，点选输出年初', async () => {
    ...
    await openPanel(wrapper)
    expect(document.querySelector('.ev-year-table')).toBeTruthy()
...
describe('EvDatePicker datetime（单值）', () => {
  it('选日期不关闭，confirm 关闭', async () => {
    ...
    // footer 确定
    const btns = [...document.querySelectorAll('.ev-picker-panel__footer .ev-picker-panel__btn')]
  ...
  it('时间按钮打开滚轮并调整时间', async () => {
    ...
    dayCell(today.date()).click()
    await flush()
    document.querySelector('.ev-date-picker__time-btn').click()
    await flush()
    expect(document.querySelector('.ev-time-panel')).toBeTruthy()
    // 点 10 时
    const hourCol = document.querySelector('.ev-time-spinner__wrapper')
    const hour10 = [...hourCol.querySelectorAll('.ev-time-spinner__item')].find((li) =>
...
describe('EvDatePicker 区间', () => {
  it('daterange：两次点选 + calendar-change + 排序输出', async () => {
    const { wrapper, picker } = mountPicker({ type: 'daterange' })
    await openPanel(wrapper)
    expect(document.querySelector('.ev-date-range-picker')).toBeTruthy()
    expect(document.querySelectorAll('.ev-date-table').length).toBe(2)
    const today = dayjs()
  ...
  it('datetimerange + default-time：选完保持打开，confirm 关闭且带时间', async () => {
    ...
    // confirm
    document.querySelector('.ev-picker-panel__footer .ev-picker-panel__btn').click()
    await flush()
    expect(document.querySelector('.ev-picker__popper')).toBeNull()
  })
  ...
  it('monthrange：月份网格区间选择', async () => {
    ...
    await openPanel(wrapper)
    expect(document.querySelectorAll('.ev-month-table').length).toBe(2)
    monthCell(1).click()
    await flush()
    monthCell(3).click()
    await flush()
  ...
  it('unlink-panels：右面板独立翻页', async () => {
    ...
    await openPanel(wrapper)
    const rightHeader = [...document.querySelectorAll('.ev-date-range-picker__header')]
    const rightLabelBefore = rightHeader[1].textContent
    // 右面板下一年
    const btns = document.querySelectorAll('.ev-picker-panel__icon-btn.d-arrow-right')
    btns[btns.length - 1].click()
    await flush()
    const rightLabelAfter = [...document.querySelectorAll('.ev-date-range-picker__header')][1].textContent
