...
import { h } from 'vue'
import EvTable from '../src/components/table/index.vue'
import EvTableColumn from '../src/components/table/column.vue'
...
function mountTable(props = {}, slots = {}) {
  return mount(EvTable, {
    props: { data: rows, ...props },
    slots: {
      default: () =>
        h('div', [
          h(EvTableColumn, { prop: 'name', label: '名称' }),
          h(EvTableColumn, { prop: 'price', label: '价格', sortable: true }),
        ]),
      ...slots,
    },
  })
...
describe('EvTable 渲染契约', () => {
  it('双 class + 结构 DOM（inner-wrapper/header-wrapper/body-wrapper/cell）', async () => {
    const wrapper = mountTable()
    await flushTable(wrapper)
    expect(wrapper.classes()).toContain('ev-table')
    expect(wrapper.classes()).toContain('ev-table')
    expect(wrapper.find('.ev-table__inner-wrapper').exists()).toBe(true)
    expect(wrapper.find('.ev-table__header-wrapper').exists()).toBe(true)
    expect(wrapper.find('.ev-table__body-wrapper').exists()).toBe(true)
    expect(wrapper.find('table.ev-table__header').exists()).toBe(true)
    expect(wrapper.find('table.ev-table__body').exists()).toBe(true)
    expect(wrapper.find('.ev-table__cell .cell').exists()).toBe(true)
  })
  ...
  it('数据行渲染 prop 字段', async () => {
    const wrapper = mountTable()
    await flushTable(wrapper)
    const trs = wrapper.findAll('tbody .ev-table__row')
  ...
  it('border/stripe 修饰类', async () => {
    ...
    await flushTable(wrapper)
    expect(wrapper.classes()).toContain('ev-table--border')
    expect(wrapper.classes()).toContain('ev-table--striped')
  })
  ...
  it('空数据显示 empty-block', async () => {
    ...
    await flushTable(wrapper)
    expect(wrapper.find('.ev-table__empty-block').exists()).toBe(true)
    expect(wrapper.find('.ev-table__empty-text').text()).toBe('暂无数据')
  })
})
...
describe('EvTable 排序', () => {
  ...
  it('实例方法 sort / clearSort', async () => {
    ...
    await flushTable(wrapper)
    expect(wrapper.find('tbody .ev-table__row .cell').text()).toContain('CPU')
    wrapper.vm.clearSort()
    await flushTable(wrapper)
    expect(wrapper.find('tbody .ev-table__row .cell').text()).toContain('苹果')
  })
...
describe('EvTable 多选', () => {
  function mountSelectionTable(props = {}) {
    return mount(EvTable, {
      ...
      slots: {
        default: () =>
          h('div', [
            h(EvTableColumn, { type: 'selection' }),
            h(EvTableColumn, { prop: 'name', label: '名称' }),
          ]),
      },
    })
  ...
  it('selection 列渲染 checkbox + 勾选触发 selection-change', async () => {
    ...
    await flushTable(wrapper)
    const boxes = wrapper.findAll('tbody .ev-checkbox')
  ...
  it('表头全选 + toggleAllSelection + clearSelection', async () => {
    const wrapper = mountSelectionTable()
    await flushTable(wrapper)
    await wrapper.find('thead .ev-checkbox input').trigger('change')
  ...
  it('selectable 禁用行不可选', async () => {
    ...
    await flushTable(wrapper)
    const firstBox = wrapper.findAll('tbody .ev-checkbox')[0]
    expect(firstBox.classes()).toContain('is-disabled')
  })
})

describe('EvTable 展开行与自定义列', () => {
  it('expand 列点击展开 + expand-change + 展开内容渲染', async () => {
    ...
    await flushTable(wrapper)
    expect(wrapper.find('.ev-table__expanded-row').exists()).toBe(false)
    await wrapper.find('.ev-table__expand-icon').trigger('click')
    await flushTable(wrapper)
    expect(wrapper.find('.ev-table__expanded-row').exists()).toBe(true)
  ...
  it('index 列序号', async () => {
    const wrapper = mount(EvTable, {
      props: { data: rows },
      slots: {
        default: () => h('div', [h(EvTableColumn, { type: 'index' }), h(EvTableColumn, { prop: 'name' })]),
      },
    })
    await flushTable(wrapper)
  ...
  it('show-overflow-tooltip 省略 + title', async () => {
    const wrapper = mount(EvTable, {
      ...
      slots: {
        default: () =>
          h('div', [h(EvTableColumn, { prop: 'name', label: '名称', showOverflowTooltip: true })]),
      },
    })
    await flushTable(wrapper)
...
describe('EvTable 筛选', () => {
  it('filter-method 过滤 + filter-change', async () => {
    ...
    await wrapper.find('.ev-table__column-filter-trigger').trigger('click')
    await flushTable(wrapper)
    expect(document.querySelector('.ev-table__filter')).toBeTruthy()
    // 点击"硬件"筛选项
    const filterItem = [...document.querySelectorAll('.ev-table__filter-list-item')].find((el) =>
      el.textContent.includes('硬件')
    )
    filterItem.click()
    await flushTable(wrapper)
    // 点击筛选按钮
    const applyBtn = [...document.querySelectorAll('.ev-table__filter-bottom button')].find((b) =>
    ...
    // 过滤后只剩 CPU 行
    expect(wrapper.findAll('tbody .ev-table__row')).toHaveLength(1)
    // clearFilter 实例方法
    wrapper.vm.clearFilter('type')
    await flushTable(wrapper)
    expect(wrapper.findAll('tbody .ev-table__row')).toHaveLength(3)
  })
})

describe('EvTable 行事件与当前行', () => {
  it('row-click / cell-click 事件', async () => {
    const wrapper = mountTable()
    await flushTable(wrapper)
    await wrapper.find('tbody .ev-table__row').trigger('click')
    expect(wrapper.emitted('row-click')).toBeTruthy()
    expect(wrapper.emitted('row-click')[0][0].name).toBe('苹果')
    // cell-click 需点击 td 本身
    await wrapper.find('tbody .ev-table__row td').trigger('click')
  ...
  it('highlight-current-row 高亮 + current-change + setCurrentRow', async () => {
    ...
    await flushTable(wrapper)
    await wrapper.findAll('tbody .ev-table__row')[1].trigger('click')
    expect(wrapper.emitted('current-change')).toBeTruthy()
    expect(wrapper.findAll('tbody .ev-table__row')[1].classes()).toContain('current-row')
    wrapper.vm.setCurrentRow(null)
    await flushTable(wrapper)
    expect(wrapper.findAll('tbody .ev-table__row')[1].classes()).not.toContain('current-row')
  })
