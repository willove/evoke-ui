...
import EbSplitter from '../src/components/splitter/index.vue'
import EbSplitterPanel from '../src/components/splitter/panel.vue'
import EbUpload from '../src/components/upload/index.vue'
import EbCalendar from '../src/components/calendar/index.vue'

describe('EbSplitter / EbSplitterPanel', () => {
  beforeEach(() => {
  ...
  const mountSplitter = (props = {}, panelProps = `default-size="30%"`) =>
    mount(
      {
        components: { EbSplitter, EbSplitterPanel },
        template: `
          <eb-splitter v-bind="props" style="width: 1000px; height: 500px;">
            <eb-splitter-panel ${panelProps}>A</eb-splitter-panel>
            <eb-splitter-panel>B</eb-splitter-panel>
          </eb-splitter>
        `,
        setup() {
          return { props }
    ...
    const wrapper = mount(
      {
        components: { EbSplitter, EbSplitterPanel },
        template: `
          <eb-splitter style="width: 1000px; height: 500px;">
            <eb-splitter-panel default-size="30%">A</eb-splitter-panel>
            <eb-splitter-panel :resizable="false">B</eb-splitter-panel>
          </eb-splitter>
        `,
      },
      { attachTo: document.body },
...
describe('EbUpload', () => {
  it('双 class + 隐藏 input（绝对定位 opacity 0）', () => {
    ...
    expect(wrapper.find('.eb-upload--text').exists()).toBe(true)
    const input = wrapper.find('input[type="file"]')
    expect(input.exists()).toBe(true)
    expect(input.classes()).toContain('eb-upload__input')
    expect(input.attributes('style') || wrapper.find('.eb-upload__input').attributes('style') || '').toBe('')
    // 样式约束在 CSS 层（position:absolute; opacity:0），DOM 断言 class 存在
  })

  it('change 选择文件后入列表并 emit', async () => {
    const wrapper = mount(EbUpload, { props: { action: '/upload', autoUpload: false }, attachTo: document.body })
    const input = wrapper.find('input[type="file"]')
    ...
    expect(wrapper.find('.eb-upload-list__item').text()).toContain('a.txt')
    wrapper.unmount()
  })

  it('multiple + limit 超限发 exceed', async () => {
    const wrapper = mount(EbUpload, { props: { action: '/upload', limit: 1, autoUpload: false }, attachTo: document.body })
    const input = wrapper.find('input[type="file"]')
    Object.defineProperty(input.element, 'files', {
  ...
  it('拖拽模式渲染 dragger', () => {
    const wrapper = mount(EbUpload, { props: { action: '/upload', drag: true } })
    expect(wrapper.find('.eb-upload-dragger').exists()).toBe(true)
    expect(wrapper.find('.eb-upload__text').exists()).toBe(true)
  })
  ...
  it('remove：点击关闭按钮移除文件', async () => {
    ...
    await input.trigger('change')
    await wrapper.find('.eb-upload-list__item-close').trigger('click')
    expect(wrapper.emitted('remove')).toHaveLength(1)
    expect(wrapper.find('.eb-upload-list__item').exists()).toBe(false)
    wrapper.unmount()
  })
  ...
  it('clearFiles expose', async () => {
    ...
    await nextTick()
    expect(wrapper.find('.eb-upload-list').exists()).toBe(false)
    wrapper.unmount()
  })
})
...
describe('EbCalendar', () => {
  it('双 class + 月视图 42 格 + 周标签', () => {
    const wrapper = mount(EbCalendar, { props: { modelValue: new Date(2026, 8, 5) } })
    expect(wrapper.find('.eb-calendar').exists()).toBe(true)
    expect(wrapper.findAll('.eb-calendar__cell')).toHaveLength(42)
    expect(wrapper.findAll('.eb-calendar__weekday')).toHaveLength(7)
    // firstDayOfWeek=1 默认：周一在首位
    expect(wrapper.find('.eb-calendar__weekday').text()).toBe('一')
  })

  it('today 与选中标记', () => {
    const wrapper = mount(EbCalendar, { props: { modelValue: new Date(2026, 8, 5) } })
    expect(wrapper.find('.eb-calendar__cell.is-today').exists()).toBe(true)
    expect(wrapper.find('.eb-calendar__cell.is-selected').exists()).toBe(true)
  })

  it('firstDayOfWeek=0 周日开头', () => {
    const wrapper = mount(EbCalendar, { props: { firstDayOfWeek: 0 } })
    expect(wrapper.find('.eb-calendar__weekday').text()).toBe('日')
  })

  it('事件渲染 + event-click', async () => {
    const wrapper = mount(EbCalendar, {
      props: {
        modelValue: new Date(2026, 8, 5),
        events: [{ date: '2026-09-05', content: '评审', type: 'danger', id: 'e1' }],
      },
    })
    const evt = wrapper.find('.eb-calendar__cell-event.is-type-danger')
  ...
  it('cell 点击更新 modelValue + select', async () => {
    const wrapper = mount(EbCalendar, { props: { modelValue: new Date(2026, 8, 5) } })
    const cells = wrapper.findAll('.eb-calendar__cell')
  ...
  it('disabledDate 拦截点击', async () => {
    ...
    await wrapper.findAll('.eb-calendar__cell')[10].trigger('click')
    expect(wrapper.emitted('update:modelValue')).toBeUndefined()
    expect(wrapper.findAll('.eb-calendar__cell')[10].classes()).toContain('is-disabled')
  })

  it('prev/next 导航 + panel-change + 标题切换', async () => {
    const wrapper = mount(EbCalendar, { props: { modelValue: new Date(2026, 8, 5) } })
    expect(wrapper.find('.eb-calendar__title').text()).toBe('2026年9月')
    await wrapper.findAll('.eb-calendar__nav-btn')[0].trigger('click')
    expect(wrapper.find('.eb-calendar__title').text()).toBe('2026年8月')
  ...
  it('年视图切换 + 点击月份回到月视图', async () => {
    ...
    await nextTick()
    expect(wrapper.find('.eb-calendar--year').exists()).toBe(true)
    expect(wrapper.findAll('.eb-calendar__year-month')).toHaveLength(12)
    await wrapper.findAll('.eb-calendar__year-month')[0].trigger('click')
    await nextTick()
    expect(wrapper.find('.eb-calendar__grid').exists()).toBe(true)
  })
  ...
  it('range 高亮', () => {
    ...
    expect(wrapper.findAll('.eb-calendar__cell.is-in-range').length).toBeGreaterThanOrEqual(7)
  })
