<template>
  <div class="ev-statistic" :style="valueStyle">
    <div v-if="$slots.title || title" class="ev-statistic__title">
      <slot name="title">{{ title }}</slot>
    </div>
    <div class="ev-statistic__content">
      <span v-if="$slots.prefix || prefix" class="ev-statistic__prefix">
        <slot name="prefix">{{ prefix }}</slot>
      </span>
      <span v-if="loading" class="ev-statistic__loading">
        <span class="ev-statistic__loading-dot" />
        <span class="ev-statistic__loading-dot" />
        <span class="ev-statistic__loading-dot" />
      </span>
      <span v-else :class="['ev-statistic__value', { 'is-flip': flip }]" :title="displayValue">
        <template v-if="flip">
          <transition name="ev-statistic-flip" mode="out-in">
            <span :key="displayValue">{{ displayValue }}</span>
          </transition>
        </template>
        <template v-else>{{ displayValue }}</template>
      </span>
      <span v-if="$slots.suffix || suffix" class="ev-statistic__suffix">
...
defineOptions({ name: 'EvStatistic' })
