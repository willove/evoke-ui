<template>
  ...
  >
    ...
    >
      <span class="eb-checkbox__inner" />
      <input
        ref="checkboxRef"
        type="checkbox"
        class="eb-checkbox__original"
        :value="label"
        :name="name"
        :checked="isChecked"
    ...
    <span v-if="hasLabel" class="eb-checkbox__label">
      <slot>{{ label }}</slot>
    </span>
  </label>
</template>
...
const sizeClass = computed(() => {
  ...
  if (s === 'large') return 'eb-checkbox--large'
  if (s === 'small') return 'eb-checkbox--small'
  return ''
})
