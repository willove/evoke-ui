...
<DemoBlock>
  <eb-alert title="恭喜你，这是一条成功消息" type="success" />
  <eb-alert title="提示的消息" type="info" description="这是一条带描述信息的提示，标题加粗、描述常规灰。" style="margin-top: 12px;" />
  <eb-alert title="警告的消息" type="warning" :closable="false" style="margin-top: 12px;" />
  <eb-alert title="错误的消息" type="error" effect="dark" style="margin-top: 12px;" />
</DemoBlock>
...
<DemoBlock>
  <eb-alert title="浅色主题（light）" type="success" show-icon style="margin-bottom: 12px;" />
  <eb-alert title="深色主题（dark）" type="success" show-icon effect="dark" />
</DemoBlock>
...
<DemoBlock>
  <eb-alert title="成功状态的居中提示" type="success" show-icon center style="margin-bottom: 12px;" />
  <eb-alert title="自定义图标与内容" type="warning" show-icon>
    <template #icon>
      <eb-icon name="warning" :size="16" />
    </template>
...
<DemoBlock>
  <eb-alert :key="alertKey" title="账户将于 7 天后过期，请及时续费" type="warning" show-icon @close="closeLog = 'close 事件已触发'" />
  <eb-button style="margin-top: 12px;" @click="alertKey++">重新显示</eb-button>
  <p style="margin: 8px 0 0; color: var(--eb-text-color-secondary); font-size: 13px;">状态：{{ closeLog }}</p>
</DemoBlock>
...
<DemoBlock>
  <eb-alert type="info" show-icon :closable="false">
    <template #title>
      内容审核未通过
    </template>
    违规原因：标题含敏感词。前往 <eb-link type="primary" style="margin: 0 4px;">修改页面</eb-link> 调整后重新提交。
  </eb-alert>
</DemoBlock>
...
<DemoBlock>
  <eb-alert title="primary 类型提示" type="primary" show-icon style="margin-bottom: 12px;" />
  <eb-alert title="info 类型提示" type="info" show-icon />
</DemoBlock>
