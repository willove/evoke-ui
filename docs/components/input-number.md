...
仅允许输入标准数字值的输入框：内置步进按钮与键盘上下键步进，min/max 自动钳制（到边界后对应按钮禁用），支持精度格式化与步长倍数约束。在 ev-form 中 change / blur 时机自动触发校验。

## 基础用法

<DemoBlock>
  <ev-input-number v-model="v" :min="1" :max="10" />
  <ev-input-number :model-value="1.5" :step="0.1" :precision="1" disabled />
</DemoBlock>
...
<DemoBlock>
  <ev-input-number v-model="stepV" :step="5" />
  <ev-input-number v-model="strictV" :step="10" step-strictly />
</DemoBlock>
...
<DemoBlock>
  <ev-input-number v-model="rangeV" :min="0" :max="100" placeholder="0 - 100" />
  <ev-input-number v-model="rangeV" :precision="2" :step="0.01" placeholder="保留两位小数" />
</DemoBlock>
...
<DemoBlock>
  <ev-input-number :model-value="2" controls-position="right" />
  <ev-input-number :model-value="8" :controls="false" />
</DemoBlock>

## 尺寸

size 支持 large / small（缺省为默认高度），位于 ev-form 内时会继承表单尺寸。

<DemoBlock>
  <ev-input-number v-model="sizeV" size="large" />
  <ev-input-number v-model="sizeV" />
  <ev-input-number v-model="sizeV" size="small" />
</DemoBlock>
...
<DemoBlock>
  <ev-input-number :model-value="3" disabled />
  <ev-input-number :model-value="6" readonly />
</DemoBlock>
...
<DemoBlock>
  <ev-input-number v-model="clearV" :min="1" :value-on-clear="1" placeholder="清空后写回 1" />
</DemoBlock>
