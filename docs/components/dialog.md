...
<DemoBlock>
<ev-button type="primary" @click="visible = true">打开对话框</ev-button>
...
<DemoBlock>
<ev-button @click="alignVisible = true">垂直居中</ev-button>
<ev-button style="margin-left: 12px;" @click="topVisible = true">顶部偏移 8vh</ev-button>
<ev-dialog v-model="alignVisible" title="居中对话框" width="420px" align-center>
  <p>align-center：水平垂直双居中，常用于二次确认场景。</p>
</ev-dialog>
<ev-dialog v-model="topVisible" title="顶部对齐对话框" width="560px" top="8vh">
...
<DemoBlock>
<ev-button @click="fullVisible = true">全屏对话框</ev-button>
<ev-button style="margin-left: 12px;" @click="headerVisible = true">自定义头部</ev-button>
<ev-dialog v-model="fullVisible" title="全屏" fullscreen>
  <p>fullscreen 为 true 时忽略 width 与 top，铺满视口，适合大表单或文档预览。</p>
</ev-dialog>
<ev-dialog v-model="headerVisible" width="480px">
  <template #header>
    <span style="font-weight: 600; color: var(--ev-color-primary);">自定义头部</span>
  </template>
...
<ev-dialog v-model="guardVisible" title="异步校验" width="440px" :before-close="guardClose">
...
<DemoBlock>
<ev-button @click="lockVisible = true">仅按钮可关闭</ev-button>
<ev-dialog v-model="lockVisible" title="受控关闭" width="440px" :close-on-click-modal="false" :close-on-press-escape="false">
  <p>点击遮罩与按 ESC 均不会关闭，请使用下方按钮。</p>
  <template #footer>
    <ev-button type="primary" @click="lockVisible = false">我已知晓，关闭</ev-button>
  </template>
</ev-dialog>
</DemoBlock>
