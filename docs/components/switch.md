...
表示两种相互对立状态的切换控件，支持自定义开 / 关时的取值与文案，以及异步加载态。在 eb-form 中 change 时机自动触发校验。

## 基础用法

<DemoBlock>
  <eb-switch v-model="v" />
  <eb-switch :model-value="true" disabled />
  <eb-switch loading />
</DemoBlock>
...
<DemoBlock>
  <eb-switch v-model="pubV" active-text="发布" inactive-text="下线" />
  <eb-switch v-model="inlineV" inline-prompt active-text="开" inactive-text="关" :width="52" />
  <eb-switch v-model="inlineV" size="small" style="margin-left: 12px;" />
  <eb-switch v-model="inlineV" size="large" style="margin-left: 12px;" />
</DemoBlock>
...
<DemoBlock>
  <eb-switch v-model="valV" active-value="yes" inactive-value="no" />
  <span style="margin-left: 12px;">当前值：{{ valV }}</span>
  <eb-switch :model-value="1" :active-value="1" :inactive-value="0" style="margin-left: 24px;" />
</DemoBlock>
...
<DemoBlock>
  <eb-switch :model-value="asyncV" :loading="switching" @change="handleAsyncChange" />
</DemoBlock>
...
<DemoBlock>
  <eb-switch v-model="adminV" active-text="仅管理员" inactive-text="所有人" :width="64" />
  <eb-input model-value="系统配置说明" :disabled="!adminV" style="margin-top: 12px;" />
</DemoBlock>
