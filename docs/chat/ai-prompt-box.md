...
界面文案（发送 / 停止 / 添加附件 / 场景 / 设置 / 选择模型 / 占位文本等）取自语言包 `eb.chat`，随 `EbConfigProvider` 的 `locale` 走——机制见 [Chatbot · 多语言](/chat/chatbot#多语言)。
...
需要完整 AI 工作台（欢迎标题 + 示例问题 + 会话流）时，直接使用编排组件 [AiConsole](/chat/ai-console)，无需自行拼装。
...
<ApiTable title="AiPromptBox Props" :rows="[
  ...
  { name: 'placeholder', desc: '占位文本；不传时取当前语言包（`eb.chat.promptBox.placeholder`）', type: 'string', default: '—' },
