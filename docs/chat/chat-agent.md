...
<DemoBlock>
  <eb-chatbot v-model="planMsgs" height="520px" :show-tip="false" @plan-step-click="onStepClick" />
...
`message.confirmation = { id, title, description?, actions: [{ key, label, type?, icon?, status? }], status, responseKey? }`，`status` 取 `pending / approved / rejected / expired`。**响应后进入只读态**并显示选了哪一项，避免用户以为还能改。

`actions[].type` 取 `EbButton` 那套（`default / primary / success / warning / info / danger / text`），**按钮本身就是 `EbButton`（`size="small"`）**——外观、焦点环、禁用态都由它承担；动作可给 `icon`（库内图标名，如批准用 `check-circle`、拒绝用 `close-circle`），**响应后那颗挑中的自动换成对勾**（「选了哪个」比「这颗是什么动作」更该被看见），卡片另加「已选中」标记（响应后那颗挑中的保留主色描边 + 对勾，其余转禁用）。
...
<DemoBlock>
  <eb-chatbot v-model="confirmMsgs" height="520px" :show-tip="false" @confirm-respond="onRespond" />
...
engine.setConfirmation(msg.id, {
  ...
  status: 'pending',
  actions: [
    { key: 'approve', label: '批准执行', type: 'primary', icon: 'check-circle', status: 'approved' },
...
<DemoBlock>
  <eb-chatbot v-model="artifactMsgs" height="520px" :show-tip="false" @artifact-open="onArtifactOpen" />
