...
const releases = [
  {
    tag: 'Unreleased',
    date: '即将发版',
    title: '体验回访：弧向 / 动效 / 比例修正 + 弧形环状 + 日历热力',
    bullets: [
      '仪表盘弧向修正：startAngle/endAngle 按数学角约定（同 ECharts），默认恢复「弧走上方、开口朝下」经典形态',
      '桑基/弦图/弧长图悬浮聚焦 220ms 缓动进出，图例焦点淡化全局 180ms 缓动；弦/韦恩主体展示区域放大，弧长图不再压标题',
      '弦图新增弧形环状形态（chordMode: curve，节点圆点 + 弧线连接）；新增日历热力图 calendar-heatmap（年月日周热度/今日描边/少多色阶）',
      'K 线叠加 MA 均线（candleMa）与量副图组合；量副图扩展到折线/面积（分时图）；甘特进度标签内嵌化；分组箱线命中修正；分面散点崩溃修复',
    ],
  },
  {
    tag: 'Unreleased',
