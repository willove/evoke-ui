...
同一份校招数据，传 `funnelMinRatio` 把尾层最小宽度抬起来：宽度不再是严格等比（数值与占比仍逐层标注），换来的是尾部两层能看清——数据量级相差两个数量级时按需取舍。

<DemoBlock>
  <ev-chart
    :options="{
      type: 'funnel',
      title: '2026 校招转化（压缩尾段）',
      funnelMinRatio: 0.12,
      funnelData: [
        { label: '收到简历', value: 4860 },
        { label: '笔试通过', value: 1620 },
...
- 段色按层序同一色相递浅（首层品牌色 → 末层最浅）：漏斗讲的是一条流程的逐层收窄，
  不是并列类目；某层要定色写 `funnelData[].color`，该层即不再参与递浅；
- 宽度即数值，末层下宽等于自身宽度——漏斗底部收成平边而不是针尖；极差悬殊
  （4860 → 212 这种）时尾层必然很窄，这是数据的真实形态，要让尾层更可读就传
  `funnelMinRatio`（0–0.5，0 = 严格等比）压缩尾段；
- 段内两行标签（名称 + 数值占比）在梯形与层高放得下时自动内嵌，文字色按段底色取
  对比度更高的一方；放不下自动转右侧引线标签，纵向 36px 最小步进防重叠；
- 标签随进度平滑淡入，形状展开带缓动收尾；悬浮层片同色加深，不位移；
- `funnelData` 按流程顺序排列，漏斗形状自动生成；`pyramid: true` 反转成金字塔；
- 悬浮显示每层数值，配合图例交互；
