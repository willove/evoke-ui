...
`radarRingFill: true` 在框架内铺**同心圆环带**（交替浓淡，参考 AntV 雷达示例），「由内到外」的层次立刻可读。悬浮任一维度标签或轴顶点：该轴线点亮、维度名加粗、各系列在该维度的顶点画实心点——「这个维度谁强」顺着高亮轴一眼比对；维度文本沿外圆分散四周，与环带同一圆心。
...
- `radarRingFill: true` 开同心圆环带底色；悬浮维度标签 / 轴顶点 / 图例项均触发对应高亮（图例悬浮走全局焦点淡化）。
