...
### @wil-works/evoke-charts — 旭日图标签布局重做

- **分支同色系配色**：每个一级分支取一个基础色相，子孙节点按深度向白色提亮
  （每层约 26%）——高饱和只留在内环细带，外圈大面积不再出现整片高饱和色；
- **叶子花瓣延伸**：层级 ≥ 2 时基础环带占半径预算 66%，最外层叶子按自身数值
  向外延伸成花瓣（数值越大越长），端部同色宽描边出圆角，外置标签跟随花瓣
  尖端——旭日图从「同心圆环」升级为「由内向外扩散」的形态；
- 标签按层级分流：深度 0 水平加粗（放得下才画）；深度 ≥ 1 **沿半径方向旋转
  排布**（左半圆自动翻转保证可读），文字在自己的环带内行走、相邻段不再压字；
- 最外层叶子的标签**外置到花瓣尖端外侧**：水平排布按角度左右取向，纵向 13px
  最小间距防重叠；`showValues` 开启时外置标签带数值；
- 半径计算为外置标签预留空间（水平 45px / 纵向 24px）；
- 放不下的段不再硬塞文字，悬浮 tooltip 始终可读完整路径与数值
