...
- **聚焦涟漪反向**：激活时色环**先从边框向外荡开至 4px（约四成时长），再贴着边框向内收拢回 0 并淡出**（1.2s，`--eb-ease-in-out` 铺满全程；环内缘全程钉在边框上，荡开时从边框长出、收拢时缩回边框消失）。此前是向四周荡开约 6px 且一边扩散一边消散（0.7s ease-out，动作全压在前三分之一，看着「一闪而过」）；
