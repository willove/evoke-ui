...
const releases = [
  {
    tag: 'v0.13.0',
    date: '2026-09-26',
    title: 'EvSection 默认定宽居中 · 整页组装规则成文',
    bullets: [
      'breaking：width 默认 full → default，不写 width 的区块从此定宽居中（1152，随 --ev-container-width 令牌），标题与主体天然同边；要通栏需显式 width="full"，且仅限出血大件（跑马灯/整宽图表/分屏滚动场景/出血大图）',
      '整页组装规则成文：标题与主体共用同一个容器；禁止只给主体内层 div 手写 max-width；宽度只来自档位令牌，更窄的阅读测宽下沉组件内部',
      '文档站按新默认值除尘：首页与主题补丁删除，案例页内层私写宽度归并档位；新增容器边界守卫（视觉断言），漏定宽与「两条左边界」两类回归即红',
    ],
  },
  {
    tag: 'v0.11.0',
