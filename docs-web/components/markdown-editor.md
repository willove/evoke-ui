...
<template>
  <EwMarkdownEditor v-model="content" />
</template>
```
...
<EwMarkdownEditor v-model="toggleContent" preview="toggle" height="220px" />

```vue
<EwMarkdownEditor v-model="content" preview="toggle" />
```
...
EwMarkdownEditor 支持**加粗**、*斜体*、\`行内代码\`与[链接](https://example.com)。
