...
`EwAvatar` 展示用户或团队头像。图片缺失或加载失败时自动回退到姓名首字（中文名取前两字、
...
<div style="display:flex; align-items:center; gap:24px; flex-wrap:wrap;">
  ...
  <EwAvatar icon="github" shape="square" />
  <EwAvatar icon="wechat" shape="square" />
</div>
...
<div style="display:flex; align-items:center; gap:16px; align-items:center;">
  <EwAvatar name="小" size="small" />
  <EwAvatar name="中" />
  <EwAvatar name="大" size="large" />
  <EwAvatar name="72" :size="72" />
</div>

```vue
<EwAvatar name="中" />
<EwAvatar :src="user.avatar" size="large" />
<EwAvatar name="自定义" :size="72" />
```

</DemoBlock>

::: tip 头像组
多人层叠展示（团队、参与者列表）用 [EwAvatarGroup](./avatar-group)，支持溢出折叠为 +N。
:::
