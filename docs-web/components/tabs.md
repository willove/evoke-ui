...
<div style="display:flex; flex-direction:column; gap:16px; align-items:flex-start;">
  ...
  />
  <p style="margin:0; font-size:13px; color:var(--ew-text-secondary);">
    当前周期：<strong style="color:var(--ew-color-primary);">{{ billing }}</strong>
    <span v-if="billingLog">（最近一次切换：{{ billingLog }}）</span>
  </p>
  <EwTabs v-model="range" size="small" :items="[{ label: '近 7 天', value: 'w' }, { label: '近 30 天', value: 'mo' }]" />
</div>
