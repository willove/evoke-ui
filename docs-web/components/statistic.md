...
`EvStatistic` 展示关键数字：用户量、可用性、评分等。数值用强字重展示体并带
...
<div style="display:flex; gap:48px; justify-content:center;">
  <EvStatistic value="120K+" label="注册用户" align="center" />
  <EvStatistic value="99.99%" label="服务可用性" align="center" />
  <EvStatistic value="4.9" label="应用商店评分" align="center" />
</div>
...
<div :key="countKey" style="display:flex; gap:48px; justify-content:center;">
  ...
  <EvStatistic value="¥68" label="客单价" align="center" animated />
</div>
...
::: tip 场景建议
数字位通常出现在定价区上方或页脚之前，搭配 [EvLogoCloud](./logo-cloud) 构成完整的信任背书段落。
