...
`EwIconGrid` 是本库的招牌组件：分类分节的致密图标网格。内置 900+ 图标展示集（动态 chunk 按需加载，
...
<EwIconGrid style="max-height:520px; overflow:auto; padding:4px;" />
...
| searchable | 展示搜索栏（内部为 [EwSearchBox](./search-box)） | boolean | `true` |
...
见 [EwIcon](./icon)。扩展采样量请调整生成脚本 `SHOWCASE_PER_CATEGORY` 后重新执行 `pnpm gen:icons`。
:::
