...
`EvSwitch` 表达二元状态（开/关），用于设置项、演示面板与筛选条件。滑块带平滑位移动效，
...
<div style="display:flex; align-items:center; gap:24px; flex-wrap:wrap;">
  <label style="display:inline-flex; align-items:center; gap:8px; font-size:13px; color:var(--ev-text-regular);">
    <EvSwitch v-model="sync" @change="onChange" /> 自动同步
  </label>
  <label style="display:inline-flex; align-items:center; gap:8px; font-size:13px; color:var(--ev-text-regular);">
    <EvSwitch v-model="publicHome" @change="onChange" /> 公开主页
  </label>
...
<div style="display:flex; align-items:center; gap:16px;">
  <EvSwitch size="small" :default-value="true" />
  <EvSwitch :default-value="true" />
  <EvSwitch size="large" />
  <EvSwitch disabled :default-value="true" />
</div>

```vue
<EvSwitch size="small" :default-value="true" />
<EvSwitch v-model="a" />
<EvSwitch size="large" v-model="b" />
<EvSwitch disabled v-model="c" />
```
